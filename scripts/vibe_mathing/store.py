...
class ResearchStore:
    ...
    def _recover_locked(self) -> None:
        ...
        for item in prepared:
            ...
            if temporary.parent != target.parent or not temporary.name.startswith(
                ...
                raise StoreError("事务日志包含未授权 temporary path")
    ...
    def rebuild_solution_view(self) -> list[str]:
        ...
        with self._locked():
            attempts = {
                item["attempt_id"]: item for item in self._read_unlocked("attempts")
            }
            solution_ids = derive_solution_ids(
                self._read_unlocked("results"), attempts, project_root=self.root
            )
            payload = {
