...
def main():
    ...
    names=['HHTShifts.'+n for n in re.findall(r'^theorem\s+(\w+)',s,re.M)]
    if len(names)!=8 or len(set(names))!=8:raise ValueError('unexpected theorem manifest')
    ...
    print('SHIFT_LEAN_PASS: 8 audited theorems; interpolation and xi backend inputs are explicit, not fully formalized.')
    return 0
