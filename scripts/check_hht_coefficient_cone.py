#!/usr/bin/env python3
"""Compile the locked coefficient-cone statements and audit every dependency."""
...
SOURCE=WORK/'HHTCoefficientCone.lean'
...
def main():
    ...
    names=['HHTCoefficientCone.'+n for n in re.findall(r'^theorem\s+(\w+)',s,re.M)]
    if len(names)!=17 or len(set(names))!=17:raise ValueError('unexpected theorem manifest')
    ...
    print('COEFFICIENT_CONE_LEAN_PASS: 17 audited finite inequalities; spectral growth and actual-xi bridge external.')
    return 0
